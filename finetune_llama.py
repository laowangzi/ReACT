...
parser.add_argument("--output_path", type=str, default="lora_path")
...
#
parser.add_argument("--train_type", type=str, default="dynamic", help='dynamic/mixed/recent')
parser.add_argument("--test_type", type=str, default="dynamic")
...
if args.train_type in ['mixed', 'recent', 'random']:
    # Fit for single card V100, increasing bs if GPU allows is OK.
    if args.K <= 15:
...
if args.train_type == "mixed":
    sample_used = args.train_size * 2
else:
    sample_used = args.train_size
print(f"Samples used: {sample_used}")
...
BATCH_SIZE = min(args.total_batch_size, sample_used)
MAX_STEPS = None
...
if args.train_size >= 20000:
    Eval_DEVIATION = int(args.train_size/20000)
...
if args.train_type in ['dynamic', 'mixed', 'range'] and args.test_type == 'dynamic':
    #load sorted df
    fp_test = f"test_{args.test_type}_p{args.p}_tpmi_alpha{args.alpha}.parquet.gz"
    DATA_PATH = {
        "train": '/'.join([data_path, f"train/train_dynamic_p{args.p}_tpmi_alpha{args.alpha}.json"]),
        "test": '/'.join([data_path, f"test/test_dynamic_p{args.p}_tpmi_alpha{args.alpha}.json"])
    }
    if args.train_type == 'mixed':
        DATA_PATH["train_recent"] = '/'.join([data_path, f"train/train_K{args.K}_recent.json"])
else:
    fp_test = f"test.parquet.gz" 
    DATA_PATH = {
        "train": '/'.join([data_path, f"train/train_K{args.K}_{args.train_type}.json"]),
        "test": '/'.join([data_path, f"test/test_K{args.K}_{args.test_type}.json"])
    }
...
if USE_8bit is True and args.use_lora:
...
if args.train_type in ['mixed']:
    data['train_recent'] = data['train_recent'].shuffle()
    data["train_recent"] = data["train_recent"].select(range(args.train_size))
    data['train'] = concatenate_datasets([data['train'], data["train_recent"]])
...
def compute_metrics(eval_preds):
    ...
    if auc > curr_best:
        ...
        print(f'Best Results Save at: ', res_dir)
    # print({'auc': auc, 'll': ll, 'acc': acc})
    return {
        'auc': auc, 
        'll': ll, 
        'acc': acc, 
...
dataset_size = len(train_data)  # Number of examples in the training dataset
